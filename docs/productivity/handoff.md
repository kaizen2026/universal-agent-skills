...
It is sourced from the active Work-Item Capsule (`handoff --work-item <id>`, or the bound harness and session) and written in the portable checkpoint schema, so arrival is the same regardless of source: the receiver inspects it with `resume --input` and adopts it with `import-legacy-checkpoint --work-item <id> --input <file>`. A legacy workspace-wide checkpoint can still be exported with `--input`, but the source is always named explicitly — a bare `handoff` is rejected rather than silently falling back to the workspace-wide file — and nothing is ever imported automatically on the other side. The export carries the capsule's real `status` (`in-progress`, `blocked`, `ready-for-review`, or `complete`, set with `checkpoint --status`); an unrecorded validation state travels as `unavailable` rather than being restamped for the current commit.
