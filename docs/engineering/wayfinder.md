...
The map and its tickets live on the repo's issue tracker, so wayfinder needs the tracker wiring that [setup-universal-agent-skills](https://github.com/kaizen2026/universal-agent-skills/blob/main/skills/engineering/setup-universal-agent-skills/SKILL.md) lays down. That step writes a "Wayfinding operations" section describing how the map, its child tickets, blocking edges, and frontier queries are expressed for GitHub, GitLab, or local markdown. Wayfinder resolves that doc through the pointer in your `CLAUDE.md` / `AGENTS.md` rather than a fixed path; with no tracker configured at all it falls back to local markdown files.
...
| `prototype` | HITL | A logic or state-model question that talking cannot settle. | [prototype](https://github.com/kaizen2026/universal-agent-skills/blob/main/skills/engineering/prototype/SKILL.md), with the built artifact linked from the ticket as an asset |
| `prototype` | HITL | A visual-direction question whose intent, audience, flows, content, platform, and constraints are already resolved. | [frontend-design](https://github.com/kaizen2026/universal-agent-skills/blob/main/skills/engineering/frontend-design/SKILL.md), with its approved contract and evidence linked from the ticket; unresolved prerequisites return to grilling or the map |
...
Underneath, it is mostly other skills wearing wayfinder's scheduling: [grilling](https://aihero.dev/skills-grilling) and [domain-modeling](https://aihero.dev/skills-domain-modeling) resolve the default ticket type, [prototype](https://github.com/kaizen2026/universal-agent-skills/blob/main/skills/engineering/prototype/SKILL.md) resolves logic and state-model tickets, [frontend-design](https://github.com/kaizen2026/universal-agent-skills/blob/main/skills/engineering/frontend-design/SKILL.md) resolves prerequisite-complete visual tickets, and [research](https://aihero.dev/skills-research) runs as a subagent so its reading never lands in your session. [handoff](https://aihero.dev/skills-handoff) is the bridge in and out — into a map from a conversation that outgrew itself, out of one when a side quest appears mid-session. For anything else, [ask-matt](https://aihero.dev/skills-ask-matt) routes over the whole set.
