...
Implementation publishes a compact result under `.agents/state/coordination/<work-item-id>/`, with source references, check/review outcomes, and Git evidence. The advisor finds the matching task, reads only the useful evidence, and prepares the next prompt. A requested watcher repeatedly checks one worker/assignment locally for up to five minutes (`--timeout 300`, or a shorter requested window) and returns one result or deadline with timing evidence, without model-driven polling. Starting the wait, handling tool results, and reviewing still use tokens. Automatic repeat windows, idle-chat wake-up, and dispatch are not connected. Start with the [advisor workflow guide](./docs/engineering/prompt-engineer.md) and [agreed design notes](./docs/plans/advisor-worker-coordination.md).
