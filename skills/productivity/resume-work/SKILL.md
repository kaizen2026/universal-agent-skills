...
If authority pointers identify a worker progress report, reconcile its checklist with the task and actual evidence. A saved active step is last-reported work, not proof a process survived interruption. For an authorized worker continuation, reuse matching native tasks or restore this task's checklist through available tools without changing host settings or unrelated lists. An adviser reads and summarizes the same progress without taking ownership or starting the next coder step.
