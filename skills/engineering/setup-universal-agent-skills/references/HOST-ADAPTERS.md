...
| Claude Code CLI/IDE | Configure the reported model context window (never inventing a 155k fallback) and the host-controlled compact-trigger percentage separately. Record idempotent pre/post lifecycle events, then reconcile and inject the session-bound Work-Item Capsule once on `SessionStart` after compaction. | Claude controls actual proactive timing; the window is unverified until a real session reports it, and is an upper bound, not an exact consumed-token trigger. |
