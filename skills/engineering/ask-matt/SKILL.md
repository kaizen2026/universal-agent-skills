...
- **Host compaction** — after checkpointing, use the host's supported compaction lifecycle. Timing and commands are host-specific; an opted-in Codex adapter records duplicate lifecycle deliveries once and automatically reconciles the session-bound capsule after compaction.
